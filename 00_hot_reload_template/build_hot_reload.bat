...
odin build src/game -strict-style -vet -debug -define:RAYLIB_SHARED=true -build-mode:dll -out:build/game.dll -pdb-name:pdbs\game_%PDB_NUMBER%.pdb > nul
...
echo Building %EXE%
odin build src/reload_exe -strict-style -vet -debug -out:build/%EXE%
IF %ERRORLEVEL% NEQ 0 exit /b 1
