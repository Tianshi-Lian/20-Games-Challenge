...
FONT_FILENAME :: "assets/fonts/font.ttf"
    ...
    if font_data, ok := os.read_entire_file(FONT_FILENAME); ok {
        ...
        if stbtt.InitFont(&fi, raw_data(font_data), 0) {
            ...
            for r, r_idx in letters {
                ...
                append(
                    &pack_rects,
                    stbrp.Rect{id = make_pack_rect_id(i32(r_idx), .Glyph), w = stbrp.Coord(w) + 2, h = stbrp.Coord(h) + 2},
                )
            }
        }
    } else {
    ...
    if tileset.pixels_size.x != 0 && tileset.pixels_size.y != 0 {
        ...
        for x in 0 ..< w {
            for y in 0 ..< h {
                ...
                append(
                    &pack_rects,
                    stbrp.Rect{id = make_pack_rect_id(make_tile_id(x, y), .Tile), w = TILE_SIZE + 2, h = TILE_SIZE + 2},
                )
            }
        }
    }
