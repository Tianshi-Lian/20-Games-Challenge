...
main :: proc() {
    f, _ := os.open(VERSION_ODIN_OUTPUT_PATH, os.O_WRONLY | os.O_CREATE | os.O_TRUNC)
    defer os.close(f)
    ...
    for a in textures {
        if strings.has_suffix(a.name, ".ase") ||
           strings.has_suffix(a.name, ".aseprite") ||
           strings.has_suffix(a.name, ".png") {
            mod, mod_err := os.last_write_time_by_name(a.fullpath)
